...
export const initStepsAnimation = () => {
    ...
    const checkDesktopSvg = () => {
      ...
      if (el && el.circles.length) {
        ...
        for (let i = 0; i < totalSteps; i++) {
          ...
          const lineDuration = 0.8; // Ускорили: было 1.3 стали 0.8 (-0.5s)
        ...
        if (el.lines[totalSteps]) {
          const finalLineDuration = 0.8; // Ускорили: было 1.3 стали 0.8 (-0.5s)
          tl.to(
            el.lines[totalSteps],
            {
  ...
  mm.add("(max-width: 767px)", () => {
    ...
    let tlMobileSvg; // Выносим ссылку наружу, чтобы убить её в return
    ...
    return () => {
      tlMobile.kill();
      if (tlMobileSvg) tlMobileSvg.kill();
    };
  });
};
